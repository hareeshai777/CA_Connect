...
const DEMO_CAS = [
  { id: "demo1", firstName: "Anil", lastName: "Kumar", bio: "Expert in corporate taxation, GST compliance, and international tax planning with 15+ years experience.", experienceYears: 15, consultationFee: 49900, averageRating: 4.9, totalReviews: 127, isAvailable: true, city: "Mumbai", state: "Maharashtra", languages: "English, Hindi, Marathi", specializations: [{ service: { name: "Income Tax Filing", slug: "income-tax-filing" } }, { service: { name: "GST Filing", slug: "gst-filing" } }, { service: { name: "Audit Services", slug: "audit-services" } }] },
  ...
  { id: "demo3", firstName: "Vikram", lastName: "Singhania", bio: "Audit and compliance specialist with deep expertise in statutory audit, internal audit, and risk management.", experienceYears: 12, consultationFee: 49900, averageRating: 4.7, totalReviews: 64, isAvailable: false, city: "Delhi", state: "Delhi", languages: "English, Hindi, Punjabi", specializations: [{ service: { name: "Audit Services", slug: "audit-services" } }, { service: { name: "Business Compliance", slug: "business-compliance" } }] },
  ...
  { id: "demo5", firstName: "Sanjay", lastName: "Agarwal", bio: "GST specialist with expert knowledge of e-commerce taxation, supply chain compliance, and input tax credit optimization.", experienceYears: 7, consultationFee: 49900, averageRating: 4.8, totalReviews: 93, isAvailable: true, city: "Hyderabad", state: "Telangana", languages: "English, Hindi, Telugu", specializations: [{ service: { name: "GST Filing", slug: "gst-filing" } }, { service: { name: "Accounting Services", slug: "accounting-services" } }] },
  { id: "demo6", firstName: "Kavitha", lastName: "Reddy", bio: "Payroll and HR compliance expert managing end-to-end payroll for SMEs and large organizations.", experienceYears: 9, consultationFee: 49900, averageRating: 4.6, totalReviews: 48, isAvailable: true, city: "Pune", state: "Maharashtra", languages: "English, Telugu, Hindi", specializations: [{ service: { name: "Payroll Services", slug: "payroll-services" } }, { service: { name: "Business Compliance", slug: "business-compliance" } }] },
];
...
export function FeaturedCAsSection() {
  ...
  useEffect(() => {
    api.get("/ca?limit=6&sortBy=averageRating").then((res) => {
      ...
      if (realCAs.length === 0) return; // keep demo

      // Normalise fee to ₹499 for display and mark as real
      const normalised = realCAs.map((ca: any) => ({
        ...ca,
        consultationFee: 49900,
        _real: true,
      }));

      // Fill remaining slots (up to 6) with demo CAs that don't share a name
      ...
      const combined = [...normalised, ...fillers].slice(0, 6);
      setCas(combined);
    }).catch(() => {});
  }, []);
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-4">
          <div>
            ...
            <motion.h2
              ...
              viewport={{ once: true }}
              className="text-4xl md:text-5xl font-bold font-heading text-gray-900"
            >
              Top CA Professionals
            </motion.h2>
          </div>
          <Button variant="outline" className="rounded-xl border-2 shrink-0" asChild>
            <Link href="/services">Book a Consultation <ArrowRight className="ml-2 w-4 h-4" /></Link>
          </Button>
        </div>
