...
import { Calendar, TrendingUp, Users, Star, ArrowRight, Clock, CheckCircle, Video, BarChart3, IndianRupee, Award } from "lucide-react";
...
export default function CADashboardPage() {
  ...
  useEffect(() => {
    api.get("/ca/my/dashboard").then((r) => setStats(r.data.data)).catch(() => {}).finally(() => setLoading(false));
  }, []);
  ...
  return (
    <div>
      ...
      <div className="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          ...
          <Button className="rounded-xl bg-brand-600 hover:bg-brand-700" asChild>
            <Link href="/ca/profile">Update Profile</Link>
          </Button>
        </div>
      </div>
      ...
      {ca?.status !== "ACTIVE" && (
        <div className="mb-6 p-4 bg-yellow-50 dark:bg-yellow-950 border border-yellow-200 dark:border-yellow-800 rounded-xl flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-yellow-700 dark:text-yellow-300">Your profile is under review. You'll be notified once approved.</p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {[
          { label: "Total Bookings", value: stats?.totalBookings ?? "—", icon: Calendar, color: "text-brand-600", bg: "bg-brand-50 dark:bg-brand-950" },
          { label: "This Month", value: stats?.thisMonthBookings ?? "—", icon: TrendingUp, color: "text-green-600", bg: "bg-green-50 dark:bg-green-950" },
          { label: "Total Earnings", value: stats ? formatCurrency(stats.totalEarnings * 100) : "—", icon: IndianRupee, color: "text-purple-600", bg: "bg-purple-50 dark:bg-purple-950" },
          { label: "Avg Rating", value: stats ? `${stats.averageRating?.toFixed(1)}★` : "—", icon: Star, color: "text-gold-600", bg: "bg-yellow-50 dark:bg-yellow-950" },
              ...
              {!stats?.upcomingBookings?.length ? (
                ...
                <div className="space-y-3">
                  {stats.upcomingBookings.map((b: any) => (
                    <div key={b.id} className="flex items-center gap-4 p-4 rounded-xl border border-border">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-medium text-sm">{b.clientProfile?.firstName} {b.clientProfile?.lastName}</p>
