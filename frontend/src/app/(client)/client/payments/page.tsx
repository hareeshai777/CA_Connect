  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        ...
        <Button variant="outline" size="sm" className="rounded-xl gap-1.5"><Download className="w-4 h-4" />Export</Button>
      </div>

      {/* Summary */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        {[
          { label: "Total Spent", value: formatCurrency(totalSpent), icon: IndianRupee, color: "text-brand-600", bg: "bg-brand-50 dark:bg-brand-950" },
          { label: "Successful", value: payments.filter(p => p.status === "SUCCESS").length, icon: CheckCircle, color: "text-green-600", bg: "bg-green-50 dark:bg-green-950" },
          { label: "Transactions", value: total, icon: CreditCard, color: "text-purple-600", bg: "bg-purple-50 dark:bg-purple-950" },
            ...
            {payments.map((p, i) => {
              ...
              return (
                <motion.div key={p.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }}
                  ...
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${p.status === "SUCCESS" ? "bg-green-50 dark:bg-green-950" : "bg-muted"}`}>
