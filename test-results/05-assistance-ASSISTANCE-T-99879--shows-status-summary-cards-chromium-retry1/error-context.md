...
- Name: 05-assistance.spec.ts >> ASSISTANCE TEAM workflow >> Documents page shows status summary cards
- Location: tests/e2e/05-assistance.spec.ts:114:7
...
Locator: locator('text=Pending').first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('text=Pending').first()
...
```ts
  17  |     await expect(page.locator("main").first()).toBeVisible();
  18  |     await page.waitForTimeout(1500); // let API load
  19  |     // At least 2 stat cards present
  20  |     const cards = page.locator("[class*='card'], [class*='Card']");
  21  |     expect(await cards.count()).toBeGreaterThan(1);
  22  |   });
  23  | 
  24  |   test("Dashboard shows workflow callout", async ({ page }) => {
